...
<body>
    ...
    if (isset($_POST['confirm']) && isset($_GET['id'])) {     
        ...
        if (!checkOwnerAnnonce($id)) {
            ...
            echo '<p>Vous ne pouvez pas supprimer cette annonce car elle ne vous appartient pas</p>';
            echo '<a class="delete-button" href="'.homeLeboncoin.'annonces/annonce.php?id=' . $id . '">Retour</a>';
            echo '</div>';
            echo '</div>';
            exit;
        } else if (checkAnnonceHasReservation($id)) {
            echo '<div class="container">';
            echo '<div class="delete-container">';
            echo '<p>Vous ne pouvez pas supprimer cette annonce car elle a une réservation en cours. <br> Vous pouvez cependant choisir de ne plus l\'afficher avec le bouton ci dessous</p>';
            echo '<a class="delete-button" href="'.homeLeboncoin.'annonces/annonce.php?id=' . $id . '">Retour</a>';
            echo '<a class="delete-button" href="'.homeLeboncoin.'annonces/desactiver.php?id=' . $id . '">Ne plus afficher</a>';
            echo '</div>';
            echo '</div>';
            exit;
