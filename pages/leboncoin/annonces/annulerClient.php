...
<body>
    ...
    if (isset($_POST['confirm']) && isset($_GET['id'])) {     
        ...
        if (!checkOwnerReservation($id)) {
            ...
            echo '<a class="delete-button" href="'.homeLeboncoin.'compte/reservation.php?id=' . $id . '">Retour</a>';
            echo '</div>';
            echo '</div>';
            exit;
            ...
            $id = $_GET['id'];
            annulerReservation($id, 7);
            header('Location: '.homeLeboncoin.'compte/reservations.php');
            exit;
        } else {
            header('Location: '.homeLeboncoin.'compte/reservation.php?id=' . $id);
            exit;
