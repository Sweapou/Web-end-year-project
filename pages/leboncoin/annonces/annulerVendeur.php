...
<body>
    ...
    if (isset($_POST['confirm']) && isset($_GET['id'])) {     
        ...
        if (!checkOwnerCommande($id)) {
            ...
            $id = $_GET['id'];
            annulerReservation($id, 6);
            header('Location: '.homeLeboncoin.'compte/reservationsannonces.php');
            exit;
        } else {
            header('Location: '.homeLeboncoin.'compte/reservation.php?id=' . $id);
            exit;
