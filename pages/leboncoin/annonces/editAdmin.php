...
//  need to be logged first and the user needs to have the prenom, nom and email filled in the database
    ...
    $error = null;
    if ( isset($_POST['nom']) && isset($_POST['description']) && isset($_POST['prix']) && isset($_POST['stock']) && isset($_POST['idreftype']) && isset($_POST['idrefcondition']) && isset($_FILES["image"]) && isset($_POST['idrefetatannonce']) ) {
        ...
        echo $_POST['idrefetatannonce'];
        $idrefetatannonce = pg_escape_string($_POST['idrefetatannonce']);
        $image = "";
        ...
        $annonce = array(
            ...
            'idrefetatannonce' => $idrefetatannonce,
        ...
        editAnnonce($annonce);
        header('Location: ../compte/admin/gestionAnnonce.php');
    ...
    echo '</div>';
    echo '<div class="form-group">';
    echo '<label for="idrefetatannonce">Etat</label>';
    echo '<select name="idrefetatannonce" id="idrefetatannonce" required>';
    $etats = getEtats();
    foreach ($etats as $etat) {
        if ($etat['idetat'] == 1) {
            continue;
        }
        if ($etat['idetat'] == $annonce['idrefetatannonce']) {
            echo '<option value="'.$etat['idetat'].'" selected>'.$etat['nometat'].'</option>';
            continue;
        }
        echo '<option value="'.$etat['idetat'].'">'.$etat['nometat'].'</option>';
    }
    echo '</select>';
    echo '</div>';
    echo '<div class="form-group">';
